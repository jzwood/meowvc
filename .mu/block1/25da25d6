...
function treeify2(parent, ignore) {
	const baseCase = { "id": path.basename(parent) }
	if (!fs.existsSync(parent)) return baseCase
	return fs.readdirSync(parent).reduce(function(tree, child) {
		if(ignore && ignore.test(child)) return tree
		const childPath = path.join(parent, child)
		const isDir = fs.statSync(childPath).isDirectory()
		tree.children = tree.children || []
		isDir ? tree.children.push(treeify(childPath)) : tree.children.push({ "id": child })
		return tree
	}, baseCase)
}
